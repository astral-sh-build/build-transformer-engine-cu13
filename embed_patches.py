...
def parse_metadata_version(value: str) -> tuple[int, ...]:
    ...
    try:
        ...
        raise ValueError(f"Unexpected Metadata-Version: {value}") from error
...
def augment_metadata(
    ...
    # Preserve the upstream description format when adding provenance.
    ...
    if content_type == "text/x-rst":
        description += (
            f"This distribution was built by Astral from "
            f"`{repo_name}@{truncated_sha} <{commit_url}>`_."
        )
    else:
        description += (
            f"This distribution was built by Astral from "
            f"[{repo_name}@{truncated_sha}]({commit_url})."
        )
...
def parse_license_file_spec(
    ...
    if (
        ...
        parser.error(
            "License file destination must be a relative path under "
            f".dist-info/licenses/: {destination}"
        )
